...
export class BadgeModule {
  ...
  <text x="40" y="16" fill="#fff" font-size="11" font-family="sans-serif" font-weight="bold">${label}</text>
  <text x="40" y="31" fill="#fff" font-size="10" font-family="sans-serif" opacity="0.85">${sub}</text>
</svg>`;
  }
  ...
  show() {
    ...
    this.modal.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
        ...
        <button id="anid-badge-close" style="background:none;border:none;font-size:24px;cursor:pointer;width:44px;height:44px;border-radius:50%;display:flex;align-items:center;justify-content:center;" aria-label="${t('close')}">&#x2715;</button>
      </div>
      <div style="text-align:center;padding:24px;background:#f5f5f5;border-radius:8px;margin-bottom:20px;">${badgeSvg}</div>
      <label style="display:block;font-size:13px;color:#666;margin-bottom:6px;font-weight:600;">${t('embedCode')}</label>
      <textarea readonly style="width:100%;height:80px;padding:10px;border:1px solid #ddd;border-radius:8px;font-size:12px;font-family:monospace;resize:none;background:#fafafa;direction:ltr;text-align:left;">${embedCode.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</textarea>
      <button id="anid-badge-copy" style="display:block;margin-top:14px;padding:10px 24px;background:#1565c0;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:14px;min-height:44px;font-weight:600;margin-inline-start:auto;">${t('copyCode')}</button>
    `;
